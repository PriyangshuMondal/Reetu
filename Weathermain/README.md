...
- **Providers are abstractions.** `server/src/providers/weather/` and
  `server/src/providers/ai/` define interfaces; the Visual Crossing and
...
| `WEATHER_API_KEY`       | yes*     | Visual Crossing API key (https://www.visualcrossing.com/weather-api) |
| `WEATHER_API_BASE_URL`  | no       | Defaults to `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services` |
...
The Visual Crossing provider uses the Timeline API for current conditions,
hourly and daily forecasts, geocoding, and provider-supplied alerts. Air-quality
data is reported as unavailable when Visual Crossing does not provide it.
...
Visit `http://localhost:3001/` after `npm run build && npm start`. For active
development, `npm run dev` also starts the Vite frontend on port 5173.
...
- Single-page app (react-router) with a sidebar on desktop and a bottom nav +
  drawer on mobile. 13 sections: Home, Detailed Forecast, AI Reetu,
...
- Reetu AI panel with suggested questions, typing indicator, and a
  configuration hint when AI is not set up.
- Farmer chat uses the editable agriculture reference in
  `client/src/lib/agricultureInfo.ts`; update that file with general farming
  guidance without changing the weather provider.
