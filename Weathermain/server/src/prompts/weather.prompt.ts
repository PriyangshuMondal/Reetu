...
const SYSTEM_PROMPT = `You are Reetu, a friendly and concise weather assistant.
...
const GENERAL_SYSTEM_PROMPT = `You are Reetu, a helpful, accurate general assistant.
...
export function buildGeneralPrompt(request: AiGeneralRequest): ChatMessage[] {
  ...
  if (request.agricultureContext && request.chatContext?.mode === "farmers") {
    contextLines.push(
      `Agriculture reference supplied by the application. Use it as general guidance, keep live weather as the source of current conditions, and do not invent crop-specific rates or safety details:\n${request.agricultureContext}`,
    );
  }
