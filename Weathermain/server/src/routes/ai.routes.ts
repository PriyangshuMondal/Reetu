    ...
    if (!deps.provider) {
      res.status(503).json({
        error: {
          ...
          message:
            "Reetu AI is not configured. Set AI_PROVIDER, AI_API_KEY (and optionally AI_MODEL / AI_BASE_URL) in the server environment to enable it.",
        },
      });
      return;
    }
    ...
    if (!deps.provider) {
      res.status(503).json({
        error: {
          code: "AI_NOT_CONFIGURED",
          message:
            "Reetu AI is not configured. Set AI_PROVIDER, AI_API_KEY in the server environment to enable it.",
        },
      });
      return;
    }
