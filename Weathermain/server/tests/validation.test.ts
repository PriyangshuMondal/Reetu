...
describe("aiWeatherRequestSchema", () => {
  ...
  it("accepts the agriculture reference for farmer chat", () => {
    const parsed = aiGeneralRequestSchema.safeParse({
      question: "Should I irrigate today?",
      agricultureContext: "Ask for crop, stage, soil, and recent rainfall.",
    });
    expect(parsed.success).toBe(true);
    expect(parsed.success && parsed.data.agricultureContext).toContain("crop");
  });
