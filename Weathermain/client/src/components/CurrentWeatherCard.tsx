      ...
      <div className="relative z-10">
          ...
          {metrics(data, units, t).map(({ label, value, icon: Icon, tint }) => {
            ...
            return (
              <button
                ...
                title={`Ask Reetu about ${label}`}
                aria-label={`Ask Reetu about ${label} (${value})`}
        ...
        {onAskMetric && (
          <p className="mt-3 flex items-center gap-1.5 text-xs text-ink-3 dark:text-white/40">
            <Sparkles size={12} aria-hidden="true" /> {t("home.tapMetric", "Tap any metric to ask Reetu about it.")}
          </p>
        )}
      </div>
    </section>
