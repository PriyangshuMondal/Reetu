...
function iconForWeatherCode(code: string): LucideIcon {
  switch (code) {
    case "01d":
      return Sun;
    case "01n":
...
/**
 * Weather icon codes are exactly 3 characters (e.g. "01d"); fall back to
 * matching the condition text when a code is not available.
 */
export function weatherIcon(icon: string, condition?: string): LucideIcon {
  if (icon.length === 3) return iconForWeatherCode(icon);
