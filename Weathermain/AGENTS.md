...
- `npm run dev` — server (`tsx watch`, port 3001) + Vite dev server (5173, `/api` proxied to 3001). Run from repo root. Built production app: `http://localhost:3001/`.
...
- Weather: `server/src/providers/weather/` — Visual Crossing Timeline provider through the `WeatherProvider` interface (+ `geocode()`, optional `airQuality()`, `AirQualityNotFound`). `weatherResponseSchema.alerts` is a REQUIRED array; provider alerts are passed through and we never invent alerts.
...
- `.env.example` documents all env vars (including `WEATHER_API_BASE_URL` and `AI_IMAGE_MODEL`); weather and AI keys are both optional at runtime (app shows config hints instead of crashing).
